...
- `/home/wwb/fusion-memory-architecture.md`
- `/home/wwb/fusion-memory-execution-plan.md`
...
- 当前机器无可见 GPU、缺少 Python ML 依赖，且剩余空间不足以舒适部署 Qwen 本地模型；详见 `docs/deployment-qwen-postgres.md`。
...
当前测试结果：69 个 unittest 通过，1 个 live Postgres 测试因未设置 `FUSION_MEMORY_POSTGRES_DSN` 按预期跳过；本地 Docker Postgres/pgvector verifier 已手动通过。
...
|---|---|---|---|
| Embedding | `DeterministicEmbedder(dimensions=1024)`，只适合本地测试 | `Qwen3EmbeddingClient`、`HTTPEmbeddingClient(endpoint, model, api_key)` | 需要准备 Python 3.11/3.12、torch/sentence-transformers、模型缓存空间、timeout/retry、batch 策略、成本记录、历史 reindex/backfill |
...
| LLM extractor | 默认 `RuleBasedExtractor` | `StructuredLLMExtractor(OpenAICompatibleLLMClient(...))` | 需要确定抽取模型、prompt、schema version、API key、retry、成本/延迟预算；CLI add 流程还没有 endpoint 参数 |
| Reranker | 默认 `LexicalCrossEncoderReranker` | `Qwen3Reranker`、`HTTPReranker(endpoint, model, api_key)` | 需要准备本地模型运行环境或 HTTP endpoint；CLI search 暂未暴露 reranker endpoint 参数 |
...
1. Qwen3 本地模型环境：Python 3.11/3.12、torch、sentence-transformers、模型缓存空间、GPU/CPU 策略。
2. Embedding provider 配置：模型名、endpoint/API、本地模型路径、向量维度、batch size、timeout、retry、历史 reindex/backfill。
3. LLM extractor 配置：模型名、endpoint、API key、抽取 prompt/schema version、温度、token 限制、错误重试、成本/延迟统计。
4. Benchmark answer/judge 配置：BEAM 1M/10M 与 LongMemEval 使用的 answer model、judge model、prompt、官方数据路径、输出保存路径。
5. Reranker 配置：是否启用 balanced/benchmark rerank、rerank model、top_n、timeout、fallback 策略。
...
短期最关键的是 embedding 和 LLM extractor。当前默认实现能跑通架构，但检索质量和抽取质量不会代表生产效果。
...
- 给 CLI 增加 runtime extractor/embedder/reranker endpoint 参数，或引入统一配置文件/env 加载。
