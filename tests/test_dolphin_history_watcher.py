...
from fusion_memory.adapters.dolphin_history_watcher import config_from_workspace, load_checkpoint, sync_history_once
from fusion_memory.api.service import MemoryService
from fusion_memory.core.models import Scope


class DolphinHistoryWatcherTests(unittest.TestCase):
    def test_config_from_workspace_uses_expected_history_and_checkpoint_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(cfg.checkpoint_path, workspace / ".fusion-memory" / "dolphin-history-watcher" / "session-1.json")
    ...
    def test_sync_history_once_persists_new_jsonl_turns_and_checkpoint(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cfg = config_from_workspace(
                ...
                session_id="session-1",
                db_path=db,
                env={"PSI_MEMORY_WORKSPACE_ID": "ws", "PSI_MEMORY_USER_ID": "u", "PSI_MEMORY_AGENT_ID": "dolphin"},
            )
            ...
            try:
                facts = memory.store.list_facts(Scope(workspace_id="ws", user_id="u", agent_id="dolphin", session_id="session-1"))
            finally:
                memory.close()
    ...
    def test_cli_sync_dolphin_history_once_outputs_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            history = workspace / "histories" / "session-1.jsonl"
            db = workspace / "memory.sqlite3"
