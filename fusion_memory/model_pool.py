...
class EndpointPool:
    ...
    def choose(self) -> str:
        ...
        with self._lock:
            self._recover_due_endpoints_locked()
    ...
    def lease(self, timeout_seconds: float) -> Iterator[str]:
        ...
        while len(attempted) < len(self.endpoints):
            endpoint = self.choose()
            attempted.add(endpoint)
            remaining = max(0.0, deadline - time.monotonic())
            state = self._states[endpoint]
            if not state.semaphore.acquire(timeout=remaining):
                last_timeout = TimeoutError(f"model endpoint pool is exhausted: {_safe_endpoint(endpoint)}")
                continue
            with self._lock:
                state.in_flight += 1
            try:
        ...
        raise last_timeout or EndpointUnavailable("no healthy model endpoints are available")
    ...
    def _recover_due_endpoints_locked(self) -> None:
        now = time.monotonic()
        for state in self._states.values():
            if state.healthy or state.ejected_at is None or now - state.ejected_at < self.recovery_seconds:
                continue
            try:
                recovered = self._health_probe(state.endpoint)
            except Exception as exc:  # A failed probe must leave the circuit open.
                state.last_error = _sanitize_error(str(exc))
                continue
            if recovered:
                self.mark_success(state.endpoint)
...
_SECRET_RE = re.compile(r"(?i)(bearer\s+|api[_-]?key[=:\s]+|token[=:\s]+)[^\s,;]+")
